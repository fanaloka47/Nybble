...
unsafe fn public_window_callback_inner(
    ...
    let callback = || match msg {
        ...
        WM_DPICHANGED => {
            ...
            // Backport of winit 488c036a (Fixes #4041): only apply the old DPI
            // re-adjustment workaround on Windows 10 (build < 22000). On Windows
            // 11+ the OS-suggested rect is already correct; recomputing + nudging
            // there is what shoved the window back over the monitor boundary and
            // caused the multi-monitor drag ping-pong.
            if util::WIN10_BUILD_VERSION.is_some_and(|version| version < 22000) {
                let suggested_ul =
                ...
                // Windows 11+: trust the OS-suggested rectangle verbatim.
